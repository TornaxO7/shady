    ...
    pub fn process(&mut self, fft_out: &[Complex32]) -> &[f32] {
        ...
        for (i, range) in self.bar_bin_indices.iter().cloned().enumerate() {
            ...
            if is_silent {
                ...
                self.bar_values[i] = {
                    let rel_change = raw_new_bar_value / prev_bar_value.max(f32::EPSILON);

                    if rel_change >= 0.1 {
                        let factor = if rel_change > 1. { 0.8 } else { rel_change };

                        let new_value =
                            (factor * raw_new_bar_value) + (1. - factor) * prev_bar_value;
